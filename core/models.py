    ...
    def quality_check(self) -> dict:
        ...
        for ri in self.riskscenario_set.all().order_by('created_at'):
            ...
            if ri.current_level < 0:
                warnings_lst.append({"msg": _("{} current risk level has not been assessed").format(
                    ri), "obj_type": "riskscenario", "object": ri})
            if ri.residual_level < 0 and ri.current_level >= 0:
                errors_lst.append({"msg": _("{} residual risk level has not been assessed. If no additional measures are applied, it should be at the same level as the current risk").format(
                    ri), "obj_type": "riskscenario", "object": ri})
            ...
            if ri.residual_level < ri.current_level or ri.residual_proba < ri.current_proba or ri.residual_impact < ri.current_impact:
                if ri.security_measures.count() == 0 and ri.residual_level >=0:
                    errors_lst.append(
